...
// 新增：引入WebRenderActivity头文件（路径根据实际项目调整）
#include "activities/htmlreader/WebRenderActivity.h"
...
// 新增：全局WebRenderActivity实例（参照SleepActivity的初始化方式）
// 与SleepActivity保持一致，传入renderer和inputManager
WebRenderActivity webRenderActivity(renderer, inputManager);
...
// 新增：跳转到WebRenderActivity的回调函数（参照onGoHome/onGoToSettings风格）
void onGoToWebRender() {
  exitActivity();
  // 注意：此处为演示，实际WebServer调用时直接用全局实例的onEnter()即可
  // 如需通过Activity跳转，可new实例（但内存紧张时建议直接调用全局实例）
  enterNewActivity(new WebRenderActivity(renderer, inputManager));
}
...
void onGoHome() {
  exitActivity();
  // 新增：最后一个参数传入onGoToWebRender（静态网址按钮的回调）
  enterNewActivity(new HomeActivity(renderer, inputManager, onGoToReaderHome, onGoToSettings, onGoToFileTransfer, onGoToWebRender));
}
